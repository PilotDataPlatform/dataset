...
async def test_move_file_should_call_background_task_and_add_file_to_processing(
    client, httpx_mock, mock_minio, dataset, background_task_requests
):
    ...
    res = await client.post(f'/v1/dataset/{dataset_id}/files', json=payload)
    assert res.status_code == 200
    processing_file = [x.get('id') for x in res.json().get('result').get('processing')]
    assert processing_file == [file_id]


async def test_move_file_to_dataset_root_folder_should_call_background_task_and_add_file_to_processing(
    client, httpx_mock, mock_minio, dataset, background_task_requests
):
    dataset_id = str(dataset.id)
    file_id = '6c99e8bb-ecff-44c8-8fdc-a3d0ed7ac067'
    ...
    payload = {'source_list': [file_id], 'operator': 'admin', 'target_geid': dataset_id}
    res = await client.post(f'/v1/dataset/{dataset_id}/files', json=payload)
    assert res.status_code == 200
    processing_file = [x.get('id') for x in res.json().get('result').get('processing')]
    assert processing_file == [file_id]
