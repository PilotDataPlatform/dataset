...
Revision ID: 85fc09674dfc
Revises: dfc9500abfb3
...
revision = '85fc09674dfc'
down_revision = 'dfc9500abfb3'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    for template in base_templates:
        found = (
            session.query(DatasetSchemaTemplate)
            .where(DatasetSchemaTemplate.system_defined.is_(True), DatasetSchemaTemplate.name == template['name'])
            .all()
        )
        if not found:
            templates.append(
                DatasetSchemaTemplate(
                    **{
                        **template,
                        'geid': str(uuid4()),
                    }
...
def downgrade():
    bind = op.get_bind()
    session = orm.Session(bind=bind)
    templates = session.query(DatasetSchemaTemplate).where(
        DatasetSchemaTemplate.system_defined.is_(True),
        DatasetSchemaTemplate.name.in_(
            ['Distribution', 'Open_minds', 'Disease', 'Contributors', 'Subjects', 'Grant', 'Essential']
        ),
    )
